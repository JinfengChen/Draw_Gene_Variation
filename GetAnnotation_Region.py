        ...
        for line in filehd:
            ...
            if len(line) > 2: 
                ...
                if unit[1].isdigit():
                    ...
                    cmd8 = 'python QTL_Gene_LOD.py --gff %s.gff --lod ../input/MPR.cross.uniq.QTL.mr.table.new --trait %s --project %s' %(prefix, unit[5], prefix)
                    cmd9 = 'python QTL_Region_Gene_heatmap.py --input %s.table --rnaseq1 %s.ColdStress.table --rnaseq2 %s.Drought.table --rnaseq3 %s.Salt.table --anno %s.annotation.brief --lod %s.LOD --output %s' %(prefix, prefix, prefix, prefix, prefix, prefix, prefix)
                    os.system(cmd1)
                    os.system(cmd2)
                    os.system(cmd3)
                    os.system(cmd4)
